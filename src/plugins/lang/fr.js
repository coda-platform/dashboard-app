const fr = {
  langCode: "fr",
  loading: "Chargement...",
  home_new_patient: "Nombre de tests effectués",
  home_new_case: "Nouveaux cas ce jour",
  home_daily_death: "Nouveaux décès ce jour",

  home_positive_rate_per_site: "Taux de positivité",
  home_new_case_per_site: "Nombre de cas positifs",
  home_hospitalisation_rate: "Hospitalisations totales",

  icu: "Occupation lits de soins intensifs",
  hospitalization: "Occupation de lits d'hôpitaux",
  home_total_occupation: "Taux d'occupation des lits total",

  length_of_stay: "Distribution de la durée de séjour par site",
  age: "Distribution de l'âge par site",
  "h/f": "Distribution du sexe par status COVID-19",

  home_positivity: " Taux de positivité total: {positivity} %",
  home_average: " Moy. mob.{average}",
  home_rt: " Taux Rt. {rate}",

  covid_cases: "Nombre de cas de covid",
  death: "Nombre de décès",
  ventilator: "Nombre de patients sous respirateur",

  titleTxt: "Sélection des données",
  langTxt: "En",
  logoutTxt: "Se déconnecter",
  siteTitleTxt: "Dépôts connectés",
  selectVariableTxt:
    "Veuillez sélectionner une variable parmis les resources communes: ",
  age_groups: "Les groupes d'âge",
  selectDaysTxt: "Jours de sélection à partir de maintenant",
  selectHospitalTxt: "Sélectionnez les hôpitaux à inclure: ",
  selectAllTxt: "Tout sélectionner",
  unselectAllTxt: "Désélectionner tout",
  selectTxt: "Sélectionner",
  total: "Tous",
  runQueryTxt: "Exécuter la requête",

  noData: "Aucune donnée disponible",

  filters: "Filtres",
  fields: " Champs",

  selectMeasuresTxt: "Mesures ",
  continuous: "Continues",
  discreet: "Discrètes",
  addNewMeasure: "+ Nouvelles mesures",

  resourceTxt: "Resources",
  selectFieldTxt: "Sélectionnez les domaines",
  resources_add: "Ajouter (+)",

  disableBreakdown: "Désactivé la stratification",
  enableBreakdown: "Activer la stratification",

  selectBreakdownTxt: "Stratification",
  selectBreakdownByTxt: "Stratifier par",
  selectResourceTypeTxt: "Type de resources",
  selectResourceAttributeTxt: "Attribut de la resource",
  selectResourcePatient: "Patient",
  selectResourceAttributeAge: "Age",
  selectResourceAttributeSex: "Sex",
  selectResourceAttributeDeathDate: "deceasedDateTime",

  breakdownStart: "début",
  breakdownEnd: "fin",
  breakdownStep: "étape",
  breakdownSteps: "étapes",
  breakdownDays: "jours",

  contTxt: "Continues",
  discTxt: "Discrètes",
  selectContTxt: "Sélectionnez les mesures continues",
  selectDiscTxt: "Sélectionnez des mesures discrètes",

  resultsTxt: "Résultats",
  tableTxt: "Tableau ",
  figureTxt: "Figure ",

  summary_age_key: "Description de l'âge des patients par site",
  summary_gender_key: "Description du sex des patients par site",
  summary_age_site_strat_key:
    "Description de l'âge des patients par site et sexe",

  summary_gender_site_key: "Description du sex des patients par site",
  summary_age_site_key: "Description de l'âge des patients par site",
  summary_deceased_strat_key: "Nombre de déces par période de 14 jours ",

  male: "homme ",
  female: "femme ",

  site: "site ",
  mean: "moyenne ",
  stdev: "stdev ",
  ci95: "ci95 ",
  count: "Nombre ",

  newSearchTxt: "Nouvelle recherche",
  summaryTxt: "Résumé",
  keyVariablesTxt: "Variables-clés",
  legendTxt: "Légende",
  meanTxt: "Moyenne",
  saveImgTxt: "Enregistrer l'image",
  tested: "Nombre Testé",
  positive: "Nombre Positif",
  admitted: "Numbre d'Admis",
  mean_age: "Âge Moyen",

  age_0_to_4: "0 à 4 ans",
  age_5_to_19: "5 à 19 ans",
  age_20_to_49: "20 à 49 ans",
  age_50_to_64: "50 à 64 ans",
  age_65_and_up: "65 et plus",

  homeTxt: "Accueil",
  statsTxt: "Stats",
  learningTxt: "Learning",
  docsApiTxt: "API",
  learningTitle: "Learning",
  federated_simulation: "Résultat fédéré - Simulation",
  activeConnectionsTxt: "Connections actives",
  noActiveConnectionsTxt: "Aucune connection active",
  requestAccessTxt: "Demande d'accès",
  hubNotConnectedTxt: "HUB Déconnecté",
  homeMetricsTxt: "HM",

  requestAccessDescriptionTxt:
    " Pour obtenir accès aux données de CODA, contactez",
  mailRequestAccessTxt:
    "mailto:adresse_a_determiner@coda.ca?subject=Demande d'accès",

  QB_name: "nom",
  QB_category: "catégorie",
  QB_sex: "sexe",
  QB_deceased: "décédé",
  QB_deceased_date: "date décès",
  QB_age: "âge",
  QB_in_stock: "en stock",
  QB_yes: "oui",
  QB_no: "non",
  QB_price: "prix",
  QB_identifier: "identifiant",

  prepareRequestTxt: "Requête de préparation",
  TrainRequestTxt: "Requête d'entrainement",
  sendPrepareTxt: "Envoyer requête de préparation",
  sendTrainTxt: "Envoyer requête d'entrainement",
  evaluateResultTxt: "Résultats d'évaluation",
  countResultTxt: "compte Résultats",
};
export default fr;
