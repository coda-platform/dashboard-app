<template>
  <v-card>
    <v-card-text :class="status">
      <v-row>
        <v-col cols="4" class="test-left"
          >{{ query.name }} - {{ query.endpoint }} - {{ status }}</v-col
        >
        <v-col cols="4" class="test-left"
          >{{ timeInMs }} ms - HTTP code {{ httpCode }}</v-col
        >
...
export default {
  ...
  methods: {
    ...
    onError(err) {
      ...
      console.error(
        `${this.query.name} failed with reason ${err.request.response}`,
      );
