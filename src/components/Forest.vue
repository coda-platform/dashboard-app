...
export default {
  ...
  mounted() {
    ...
    GeneralApi.data()
      ...
      .then((data) => {
        console.log(data);
        that.forestPlot(data.plotConfig, data.data);
      });
  },
  created() {},
  methods: {
    _get_effect(d) {
      if (
        d.effect.effect !== undefined &&
        d.effect.low !== undefined &&
        d.effect.high !== undefined
      ) {
        return [d.effect.low, d.effect.effect, d.effect.high];
      }
      ...
      throw (
        "Invalid effect size reprensentation. Expected either 'effect' and " +
    ...
    forestPlot(conf, data) {
      ...
      const rows = table
        ...
        .attr("transform", function (d, i) {
          return translate(0, layout.rowHeight * i);
        });
      ...
      rows
        ...
        .attr("dx", function (d) {
          return (
            layout.padding.left + (d.descriptionOffset || 0) * layout.tabWidth
          );
        });
      ...
      let xAxis = d3.axisBottom(x).ticks(config.nTicks);
      ...
      trees
        ...
        .attr("y", function (d) {
          return layout.rowHeight / 2 - d.r / 2;
        })
