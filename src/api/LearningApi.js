...
async function getPrepare(body, sitesUri) {
  ...
  try {
    return await AxiosInstance.post(
      `learning/prepare?sites=${sitesUri}`,
      body,
      { headers: headers, timeout: 5 * 60 * 1000 },
    );
  } catch (err) {
...
async function getTrain(body, sitesUri) {
  const headers = TokenBearerHeaderFactory.get();
  try {
    return await AxiosInstance.post(`learning/train?sites=${sitesUri}`, body, {
      headers: headers,
      timeout: 5 * 60 * 1000,
    });
  } catch (err) {
...
async function getProgress(body, sitesUri) {
  ...
  try {
    return await AxiosInstance.post(
      `learning/progress?sites=${sitesUri}`,
      body,
      { headers: headers },
    );
  } catch (err) {
...
async function getEvaluate(body, sitesUri) {
  ...
  try {
    return await AxiosInstance.post(
      `learning/evaluate?sites=${sitesUri}`,
      body,
      { headers: headers },
    );
  } catch (err) {
