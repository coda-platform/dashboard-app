...
function nsummary(sitesUri, varUri, breakdownUri) {
  ...
  return AxiosInstance.get(
    `api/nsummary?sites=${sitesUri}&var=${varUri}&breakdown=${breakdownUri}`,
    { headers: headers },
  );
}
...
function mockStats(sitesUri, cont, disc, resources, breakdown) {
  const headers = TokenBearerHeaderFactory.get();
  return AxiosInstance.get(
    `api/mockStats?sites=${sitesUri}&cont=${cont}&disc=${disc}&resources=${resources}&breakdown=${breakdown}`,
    { headers: headers },
  );
}
...
async function isConnected() {
  ...
  return Promise.race([
    fetch(process.env.CODA_DASHBOARD_API_URL),
...
function customRequest(request, sites) {
  try {
    console.assert(
      ...
      "Incorrect site parameter",
    );
    const payload = window.btoa(JSON.stringify(request));
    const url = `stats/query?sites=${sites
      .map((s) => s.trim())
  ...
  selectors: [
    {
      ...
      fields: [
        // Must be in right order to match with underscore
        { path: "age" },
        { path: "gender" },
      ],
    },
